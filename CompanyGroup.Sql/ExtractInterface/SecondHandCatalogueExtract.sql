...
SET NOCOUNT ON
	...
	WHERE SecondHandStock.Quantity > 0 AND SecondHandStock.Price > 0 AND Invent.ItemId NOT IN( SELECT C.ItemId FROM axdb_20120614.dbo.InventTable as C WHERE C.DataAreaId IN ('bsc', 'hrp') AND 
																																							 C.WEBARUHAZ = 1 AND 
