...
SET NOCOUNT ON
	...
	IF (@LastVersion = 0)
	BEGIN
		SET @LastVersion = ISNULL((SELECT MAX(LastVersion) FROM ExtractInterface.dbo.SyncMetadata WHERE TableName = 'SalesLine' AND [Status] = 1), 0);
	END
	...
	AS (
		...
		ct.InventTransId, 
		ct.[DataAreaId]
		FROM Axdb_20130131.dbo.SALESLINE as L 
		RIGHT OUTER JOIN changetable(changes Axdb_20130131.dbo.SALESLINE, 0) as ct
	...
	AS  ( select CTE.[Version], 
				 ...
				 from Axdb_20130131.dbo.SALESLINE as sl
				 INNER JOIN ChangeTableCTE as CTE on CTE.InventTransId = sl.InventTransId and CTE.DataAreaId = sl.DataAreaId
				 INNER JOIN Axdb_20130131.dbo.InventTrans as it ON sl.DataAreaID = it.DataAreaID and sl.InventTransId = it.InventTransId
				 INNER JOIN Axdb_20130131.dbo.InventDim AS id ON id.InventDimId = sl.InventDimId AND id.DataAreaId = sl.DataAreaId
				 LEFT OUTER JOIN Axdb_20130131.dbo.UPDKEPEK as Picture ON Picture.ItemId = sl.ItemId and Picture.ELSODLEGESKEP = 1
...
/*
select top 1 * from Axdb_20130131.dbo.SalesLine
select * from changetable(changes Axdb_20130131.dbo.SalesLine, 0) as ct
