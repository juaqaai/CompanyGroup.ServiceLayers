...
namespace CompanyGroup.Domain.WebshopModule
{
    ...
    public class Pager
    {
        ...
        public Pager(int currentPageIndex, long count, int itemsOnPage)
        {
            ...
            //decimal d = (itemsOnPage > 0) ? (count / itemsOnPage) : 0;
            
            //utolsó oldalindex kalkulálása
            int pageCount = (itemsOnPage > 0) ? Convert.ToInt32(count / itemsOnPage) : 0;
                    
            this.LastPageIndex = ( ( pageCount * itemsOnPage ) < count ) ? pageCount + 1 : pageCount;//(itemsOnPage > 0) ? Convert.ToInt32(Math.Ceiling(d)) : 0;

            //size: 
            //int size = Math.Min( this._iItemsOnPage, this._iItemCount - this.From );

            // from:
            //int i = ( currentPageIndex - 1 ) > 0 ? currentPageIndex - 1 : 0; 
            //return ((i * this._iItemsOnPage) + 1); 
